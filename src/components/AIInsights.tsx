
import { useState, useEffect } from "react";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
import { Search } from "lucide-react";
import useWebSocket from "@/hooks/useWebSocket";

interface Anomaly {
  id: string;
  ts: string;
  host: string;
  app: string;
  severity: string;
  msg: string;
  score: number;
}

interface ForecastDataPoint {
  date: string;
  predicted: number;
  lower: number;
  upper: number;
}

const AIInsights = () => {
  const [anomalies, setAnomalies] = useState<Anomaly[]>([]);
  const [nlQuery, setNlQuery] = useState("");
  const [isNlProcessing, setIsNlProcessing] = useState(false);
  const [forecastData, setForecastData] = useState<ForecastDataPoint[]>([]);
  const [isLoadingForecast, setIsLoadingForecast] = useState(false);
  const { lastMessage } = useWebSocket("/api/ws/anomalies");

  useEffect(() => {
    if (lastMessage) {
      try {
        const newAnomaly = JSON.parse(lastMessage);
        setAnomalies(prev => [newAnomaly, ...prev.slice(0, 19)]);
      } catch (error) {
        console.error("Failed to parse anomaly message:", error);
      }
    }
  }, [lastMessage]);

  useEffect(() => {
    const loadForecastData = async () => {
      setIsLoadingForecast(true);
      try {
        // Mock API call for demonstration
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // Mock forecast data
        const mockForecast: ForecastDataPoint[] = [
          { date: "2023-06-01", predicted: 120, lower: 100, upper: 140 },
          { date: "2023-06-02", predicted: 132, lower: 110, upper: 154 },
          { date: "2023-06-03", predicted: 101, lower: 80, upper: 122 },
          { date: "2023-06-04", predicted: 134, lower: 110, upper: 158 },
          { date: "2023-06-05", predicted: 90, lower: 70, upper: 110 },
          { date: "2023-06-06", predicted: 230, lower: 180, upper: 280 },
          { date: "2023-06-07", predicted: 210, lower: 170, upper: 250 },
        ];
        
        setForecastData(mockForecast);
      } catch (error) {
        console.error("Failed to load forecast data:", error);
      } finally {
        setIsLoadingForecast(false);
      }
    };
    
    loadForecastData();
  }, []);

  const handleNlSearch = async () => {
    if (!nlQuery.trim()) return;
    
    setIsNlProcessing(true);
    try {
      // Mock API call for demonstration
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Mock response - this would navigate or update UI in real implementation
      console.log("Natural language query processed:", nlQuery);
      // In a real implementation, this would set some state or navigate to search results
    } catch (error) {
      console.error("Failed to process natural language query:", error);
    } finally {
      setIsNlProcessing(false);
    }
  };

  const formatDate = (dateString: string) => {
    const date = new Date(dateString);
    return date.toLocaleDateString();
  };

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card className="md:col-span-2">
        <CardHeader>
          <CardTitle>Natural Language Search</CardTitle>
          <CardDescription>
            Ask questions about your logs in plain English
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex space-x-2">
            <Input
              placeholder="e.g., 'Show me all error logs from the database server in the last hour'"
              value={nlQuery}
              onChange={(e) => setNlQuery(e.target.value)}
            />
            <Button onClick={handleNlSearch} disabled={isNlProcessing || !nlQuery.trim()}>
              <Search className="mr-2 h-4 w-4" />
              {isNlProcessing ? "Processing..." : "Search"}
            </Button>
          </div>
        </CardContent>
      </Card>
      
      <Card>
        <CardHeader>
          <CardTitle>Log Volume Forecast (7 Days)</CardTitle>
          <CardDescription>
            Predicted daily log volume for the next week
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingForecast ? (
            <div className="h-64 flex items-center justify-center">
              <p>Loading forecast data...</p>
            </div>
          ) : forecastData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart
                data={forecastData}
                margin={{ top: 10, right: 30, left: 0, bottom: 0 }}
              >
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis 
                  dataKey="date" 
                  tickFormatter={formatDate}
                />
                <YAxis />
                <Tooltip 
                  labelFormatter={value => formatDate(value)} 
                  formatter={(value) => [Number(value).toLocaleString(), ""]}
                />
                <Area 
                  type="monotone" 
                  dataKey="upper" 
                  stroke="#8884d8" 
                  fillOpacity={0.1} 
                  fill="#8884d8" 
                  strokeWidth={1}
                />
                <Area 
                  type="monotone" 
                  dataKey="predicted" 
                  stroke="#8884d8" 
                  fillOpacity={0.6} 
                  fill="#8884d8" 
                  strokeWidth={2}
                />
                <Area 
                  type="monotone" 
                  dataKey="lower" 
                  stroke="#8884d8" 
                  fillOpacity={0.1} 
                  fill="#8884d8" 
                  strokeWidth={1}
                />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-64 flex items-center justify-center">
              <p className="text-gray-500">No forecast data available</p>
            </div>
          )}
        </CardContent>
      </Card>
      
      <Card>
        <CardHeader>
          <CardTitle>Live Anomalies</CardTitle>
          <CardDescription>
            Unusual log patterns detected in real-time
          </CardDescription>
        </CardHeader>
        <CardContent>
          {anomalies.length > 0 ? (
            <div className="space-y-3 max-h-[300px] overflow-auto">
              {anomalies.map((anomaly) => (
                <div 
                  key={anomaly.id} 
                  className="p-3 bg-red-50 border border-red-100 rounded-md"
                >
                  <div className="flex justify-between items-start mb-1">
                    <div className="font-medium">{anomaly.host} - {anomaly.app}</div>
                    <Badge variant="outline" className="ml-2 border-red-300 text-red-700">
                      Score: {anomaly.score.toFixed(2)}
                    </Badge>
                  </div>
                  <div className="text-sm font-mono">{anomaly.msg}</div>
                  <div className="text-xs text-gray-500 mt-1">
                    {new Date(anomaly.ts).toLocaleString()}
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="h-64 flex items-center justify-center">
              <p className="text-gray-500">No anomalies detected yet</p>
            </div>
          )}
        </CardContent>
      </Card>
    </div>
  );
};

export default AIInsights;
